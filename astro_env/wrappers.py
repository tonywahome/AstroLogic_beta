...
class FlattenMultiDiscreteToDiscrete(gym.ActionWrapper):
    ...
    For MultiDiscrete([5, 3, 3, 4, 2]):
        Total combinations = 5 * 3 * 3 * 4 * 2 = 360
        Flat action 0   -> [0, 0, 0, 0, 0]
        Flat action 359 -> [4, 2, 2, 3, 1]
    """
...
class ReducedDiscreteWrapper(FlattenMultiDiscreteToDiscrete):
    """Alias for FlattenMultiDiscreteToDiscrete.

    Roll was removed from the base action space (MultiDiscrete([5,3,3,4,2])),
    so this wrapper now simply flattens the 5-dim MultiDiscrete to Discrete(360),
    identical to FlattenMultiDiscreteToDiscrete. Kept for backward compatibility
